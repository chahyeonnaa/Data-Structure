#include<stdio.h>
#include<stdlib.h>
#define FALSE 0 // visit flag에서 쓰기 위해 설정한 값
#define TRUE 1
#define MAX_VERTEX 10 // 최대 vertex의 수

typedef struct Node { //vertex를 구성하는 구조체
	int key;
	struct Node* link;
} listNode;
...
typedef struct graph {
	...
	int iden[MAX_VERTEX]; // vertex의 존재 확인에 쓰이는 배열
}graph;
...
void InsetEdge(graph* h, int connect1, int connect2); // edge 연결
...
int main()
{
	...
	char command;
	int key;
	int connect1, connect2; // 연결하고자 하는 vertex_InsertEdge 함수에서 사용
	...
	do {
		...
		case 'd': case 'D': // dfs
			...
			scanf("%d", &key);
			dfs(h, key);
			for (int i = 0; i < MAX_VERTEX; i++) // 방문 배열을 초기화해줘야 다음에 dfs를 정상적으로 쓸 수 있음
			{
				visited[i] = FALSE;
			}
			break;
		...
		case 'b': case 'B': // bfs
			...
			scanf("%d", &key);
			bfs(h, key);
			for (int i = 0; i < MAX_VERTEX; i++) // 방문 배열을 초기화해줘야 다음에 bfs를 정상적으로 쓸 수 있음
			{
				visited[i] = FALSE;
			}
			front = NULL;
...
graph* Initialize(graph* h) // 그래프 초기화
{
	if (h != NULL) // 그래프가 비어있지 않다면,
	{
		...
		printf("To reuse a graph, you must initialize it one more time.");
		// 그래프를 다시 쓰려면, z를 한번 더 눌러서 공간 마련을 해야함.
		return NULL;
	}
	else
	{
...
void InsertVertex(graph* h, int key) // vertex 삽입
{
	h->iden[key] = 1; // 삽입된 vertex는 확인값을 주어 vertex의 존재성 확인
	if (key > 9 || key < 0) // vertex 번호, 개수 제한
		printf("Vertex number exceeded\n");

}

void InsetEdge(graph* h, int connect1, int connect2) // 간선 연결
{
	listNode* trail = NULL;
	...
	else // 해당 인접리스트가 무언가를 가리키고 있는 상태라면,
	{
		...
		if (trail->key > connect1)// 들어가고자하는 vertex가 가장 처음에 삽입되어야한다면,
		{
		...
		else // vertex 삽입 위치 찾기(번호가 작은 vertex부터 탐색되어야함)
		{
			...
			{
				if (trail->link == NULL || trail->link->key > connect1) // 가장 마지막에 삽입되어야하거나, 위치를 찾았다면
					break; // 반복문 탈출
				trail = trail->link;
			}
	...
	vertex2->key = connect2; // 공간 할당 후, 데이터 삽입
	vertex2->link = NULL;
	...
	else // 해당 인접리스트가 무언가를 가리키고 있는 상태라면,
	{
		...
		if (trail2->key > connect2) // 들어가고자하는 vertex가 가장 처음에 삽입되어야한다면,
		{
			vertex2->link = trail2;
			h->list[connect1] = vertex2; // trail을 이용하여 인접리스트에 바로 연결해주기
			return;
		}
		...
		else // vertex 삽입 위치 찾기(번호가 작은 vertex부터 탐색되어야함)
		{
			...
			{
				if (trail2->link == NULL || trail2->link->key > connect2) // 가장 마지막에 삽입되어야하거나, 위치를 찾았다면
					break;
				trail2 = trail2->link;
...
void printgraph(graph* h) // 그래프 상황 출력
{
	...
	for (int i = 0; i < MAX_VERTEX; i++)
	{
		if (h->iden[i] == 1) // 삽입되어있는 vertex들만 출력
		{
...
void dfs(graph* h, int key) // 깊이 우선 탐색_ 시작 정점을 key값으로 받음
{
	...
	if (h->iden[key] != 1) // 입력된 vertex의 존재 확인
	{
		printf("vertex is not exist\n");
		return;
	}
...
void bfs(graph* h, int key) // 너비 우선 탐색_ 시작 정점을 key값으로 받음
{
	if (h->iden[key] != 1) // vertex의 존재확인
	{
		printf("vertex is not exist\n");
		return;
	}
...
listNode* deQueue() // 너비 우선 탐색에 필요한 과정_ 데이터 삭제
{
	if (front == rear)
	{
		printf("\nQueue is empty!\n");
...
void enQueue(listNode* aNode) // 너비 우선 탐색에 필요한 과정_ 데이터 삽입
{
